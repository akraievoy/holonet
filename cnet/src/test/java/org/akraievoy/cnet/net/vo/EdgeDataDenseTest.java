...
public class EdgeDataDenseTest extends TestCase {
  ...
  public void testJsonSerialization() throws IOException {
    ...
    final EdgeData res = new EdgeDataDense().fromStream(new ByteArrayInputStream(bytes));

    final byte[] bytes2 = ByteStreams.toByteArray(res.createStream());

    assertTrue(Arrays.equals(bytes, bytes2));

    final EdgeData res2 = new EdgeDataDense().fromStream(new ByteArrayInputStream(bytes2));
