...
object AttackMini extends App {
    ...
    for (a <- 0 until attackNum) {
      performAttack(a, eClone(links)){
        case (linksAttack, lambdaIndex, attFrac) =>
          ...
          println(
            Seq(distExp, powExp, lambdaInit, effInit, connInit, attFrac, lambdaAttack, effAttack, connAttack).mkString(";")
          )
      }
    }
  }
  ...
  def performAttack(attackNum: Int, linksAttack: EdgeData)(reportFun: (EdgeData, Seq[Int], Double) => Unit): Unit = {
  ...
  def initLinks(powExp: Double, distExp: Double, linkESource: EntropySourceRandom): Any = {
    links.clear()
    ...
    do {
      ...
      if (linkModel.getSize > 0) {
        val linkId = linkModel.generate(linkESource, true, null)
